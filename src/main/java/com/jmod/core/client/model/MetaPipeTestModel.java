...
public class MetaPipeTestModel extends MetaBlockModel{
    ...
    @Override
    public int getVariantFromState(@NotNull IExtendedBlockState state) {
    ...
    @Override
    public int getVariantFromItem(@NotNull ItemStack stack) {
        return ITEM_VARIANT;
    }
    ...
    private BakedQuad createOverlayQuad(@NotNull EnumFacing side, @NotNull EnumFacing positionSide, Vector3f from, Vector3f to){
        float offset = 0.01f;
    ...
    private @NotNull BakedQuad[] createCenterPartialQuads(){
    ...
    private void generateQuadsForSide(@NotNull EnumFacing side){
        generateQuadsForSide(this.from[side.getIndex()], this.to[side.getIndex()], side);
    }

    private void generateQuadsForSide(@NotNull Vector3f from, @NotNull Vector3f to, @NotNull EnumFacing side){
    ...
    private @NotNull BakedQuad createCenterPipeQuad(@NotNull Vector3f from, @NotNull Vector3f to, @NotNull EnumFacing side){
    ...
    private @NotNull BakedQuad createSidedPipeQuad(@NotNull Vector3f from, @NotNull Vector3f to, @NotNull EnumFacing side){
        return ModelUtils.createQuad(side, Minecraft.getMinecraft().getTextureMapBlocks()
                .getAtlasSprite("jmod:block/pipe"), 10, from, to,
                resizeUVForLargerTexture(ATLAS_SIZE, getEndArmUVFromPipeSize(this.pipeSize)));
    }

    private @NotNull BakedQuad createArmPipeQuad(@NotNull Vector3f from, @NotNull Vector3f to, @NotNull EnumFacing side, EnumFacing positionSide){
        BakedQuad quad = ModelUtils.createQuad(positionSide, Minecraft.getMinecraft().getTextureMapBlocks()
                        .getAtlasSprite("jmod:block/pipe"), 10, from, to,
